...
// Optimizado: se pasa el contenedor como puntero (const Container *c) para no copiar la estructura en cada recursión
float calc_cota(int idx, float p_act, float v_act, float val_act, Paquete *paqs, int n, const Container *c) {
...
void b_and_b(int idx, float p_act, float v_act, float val_act, bool *sel, Paquete *paqs, int n, const Container *c) {
    if (idx == n) {
        #pragma omp critical
        {
            if (val_act > max_val) {
    ...
    // Lectura atómica para evitar comportamiento indefinido al leer una variable compartida en OpenMP
    float current_max;
    #pragma omp atomic read
    current_max = max_val;
    
    if (calc_cota(idx, p_act, v_act, val_act, paqs, n, c) <= current_max) return; // Poda
...
int main() {
    ...
    for (int v = 0; v < 3; v++) {
        ...
        for (int c = 0; c < 3; c++) {
            ...
            // Resetear las variables globales y limpiar el arreglo de resultados
            max_val = best_peso = best_vol = 0;
            memset(best_comb, 0, sizeof(best_comb));

            printf("Container: %.0f m3\n", conts[c].vol_max);

            // Paralelización inicial OpenMP
            #pragma omp parallel sections
            {
                #pragma omp section
                {
                    ...
                    if (p_v[0].peso <= conts[c].peso_max && p_v[0].vol <= conts[c].vol_max) {
                        sel[0] = true; 
                        b_and_b(1, p_v[0].peso, p_v[0].vol, p_v[0].v_efec, sel, p_v, n, &conts[c]);
                    }
                }
                #pragma omp section
                {
